...
def main():
    ...
    # Print CRS of the original mo_basin
    print(f"CRS of mo_basin: {mo_basin.crs}")
    ...
    buffer_size = 1000000  # Buffer size in degrees; adjust as needed
    mo_basin_buffered = mo_basin.buffer(buffer_size)

    # # Re-project to a projected CRS (e.g., EPSG:3857)
    # mo_basin_projected = mo_basin.to_crs(epsg=3857)

    # # Apply a buffer to the re-projected Missouri River Basin geometry
    # buffer_size = 1000000  # Buffer size in meters; adjust as needed
    # mo_basin_buffered = mo_basin_projected.buffer(buffer_size)

    # # Re-project back to the original geographic CRS
    # mo_basin_buffered = mo_basin_buffered.to_crs(epsg=4326)
