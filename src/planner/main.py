...
from constellation_config_files.schemas import SNODASStatus, SWEChangeLayer, VectorLayer
...
class Environment(Observer):
    ...
    *The Environment object class inherits properties from the Observer object class in the NOS-T tools library*
    ...
    def generate_combined_dataset(self, dataset1, dataset2, mo_basin):
        ...
        new_ds1 = self.interpolate_dataset(dataset1, variables_to_interpolate, lat_coords, lon_coords, time_coords_ds1)
        new_ds2 = self.interpolate_dataset(dataset2, variables_to_interpolate, lat_coords, lon_coords, time_coords_ds2)
    ...
    def combine_and_multiply_datasets(self, ds, eta5_file, eta0_file, eta2_file, weights, output_file):
        """
    ...
    def encode(self, dataset, variable, output_path, time_step, scale, geojson_path, downsample_factor=1, rotate=False):
        ...
        try:
            if rotate:
                bottom_left, bottom_right, top_left, top_right = self.get_extents(dataset, variable=variable)
            else:
    ...
    def on_change(self, source, property_name, old_value, new_value):
        ...
        *Standard on_change callback function format inherited from Observer object class*
        ...
        if property_name == Simulator.PROPERTY_MODE and new_value == Mode.EXECUTING:
            ...
            session_token, secret_access_key, access_key_id = self.decompose_token(token)
            ...
            mo_basin = self.get_missouri_basin(file_path="data/Downloaded_files/Mo_basin_shp/WBD_10_HU2_Shape/Shape/WBDHU2.shp") #"WBD_10_HU2_Shape/Shape/WBDHU2.shp")

            # Load the US states boundary
            conus = self.get_us_states(file_path="https://www2.census.gov/geo/tiger/GENZ2018/shp/cb_2018_us_state_20m.zip")
            ...
            swe_layer_encoded, top_left, top_right, bottom_left, bottom_right = self.encode(
            ...
            all_json_data = final_eta_gdf_clipped.drop('time', axis=1, errors='ignore').to_json()
            self.app.send_message(
                self.app.app_name,
                "all",
                VectorLayer(
                    vector_layer=all_json_data
                ).json(),
            )
            ...
            # Clip Final Eta GDF and ground tracks to the Missouri Basin
            selected_cells_gdf_clipped = selected_cells_gdf #gpd.clip(selected_cells_gdf, mo_basin)
            
            # Convert the clipped GeoDataFrame to GeoJSON and send as message
            selected_json_data = selected_cells_gdf_clipped.drop('time', axis=1, errors='ignore').to_json()
            self.app.send_message(
                self.app.app_name,
                "selected",
                VectorLayer(
                    vector_layer=selected_json_data
                ).json(),
            )
