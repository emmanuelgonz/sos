...
def interpolate_dataset(dataset, variables_to_interpolate, lat_coords, lon_coords, time_coords):
    # Create meshgrid for interpolation
    xi = np.stack(np.meshgrid(lon_coords, lat_coords), axis=2).reshape((lat_coords.size * lon_coords.size, 2))
...
def generate_combined_dataset(dataset1, dataset2):
    ...
    new_ds1 = interpolate_dataset(dataset1, variables_to_interpolate, lat_coords, lon_coords, time_coords_ds1)
    new_ds2 = interpolate_dataset(dataset2, variables_to_interpolate, lat_coords, lon_coords, time_coords_ds2)
    ...
    last_date_ds1 = np.datetime_as_string(dataset1.time[-1].values, unit='D').replace('-', '')
    last_date_ds2 = np.datetime_as_string(dataset2.time[-1].values, unit='D').replace('-', '')
    print(last_date_ds1)
    print(last_date_ds2)
    ...
    clipped_dataset = combined_dataset.rio.clip(mo_basin.geometry, all_touched=True, drop=True)
    ...
    last_date = str(combined_dataset.time[-1].values)[:10].replace("-", "")  # Format as 'yyyymmdd'
    print(last_date)
...
def generate_swe_difference(ds):
    ...
    eta5_values = calculate_eta(swe_diff_abs)
...
def generate_surface_temp(ds):
    ...
    print("Surface Temperature min:", surface_temp.min().values, "max:", surface_temp.max().values)
    ...
    print("Temperature eta0 min:", eta0_values.min().values, "Temperature eta0 max:", eta0_values.max().values)
...
def generate_sensor_gcom(ds):
    # Extract the SWE variable
    swe = ds['SWE_tavg']

    # Check the range of SWE values
    print("SWE min (combined):", swe.min().values, "SWE max (combined):", swe.max().values)
    ...
    print("Eta2 min:", eta2_values.min().values, "Eta2 max:", eta2_values.max().values)
...
def generate_sensor_capella(ds):
    ...
    capella_ds = xr.Dataset({'eta2': eta2_capella_values}).transpose('time', 'y', 'x')
...
def combine_and_multiply_datasets(ds, eta5_file, eta0_file, eta2_file, weights, output_file):
    """
...
def process(ds, mo_basin, gcom_ds, capella_ds):
    ...
    capella_tracks['time'] = pd.to_datetime(capella_tracks['time']).dt.tz_localize(None)
    gcom_tracks['time'] = pd.to_datetime(gcom_tracks['time']).dt.tz_localize(None)

    # Select the ground tracks for the specific date
    capella_tracks = capella_tracks[capella_tracks['time'] == pd.to_datetime("2019-03-10")]
    ...
    final_eta = final_eta.where(~gcom_mask, capella_eta - gcom_eta)  # Subtract where overlap
    final_eta = final_eta.where(~capella_mask, capella_eta)  # Retain Capella-only values
    final_eta = final_eta.where(capella_mask, np.nan)  # Assign NaN outside Capella coverage
...
def find_optimal_solution(final_eta_gdf):
    ...
    final_eta_gdf["final_eta"] = final_eta_gdf["final_eta"].replace([np.inf, -np.inf], np.nan)
    ...
    objective = lpSum(x[i] * final_eta_gdf.loc[i, "final_eta"] for i in final_eta_gdf.index)
    model += objective
...
def get_data_geojson(s3_filepath):
    """
    Get data from an S3 bucket
    ...
    # Get the file contents
    fgrab = fs.open(s3_filepath).read()

    final_eta_gdf = gpd.read_file(BytesIO(fgrab))

    return final_eta_gdf

def get_data_geojson_alternative(s3, bucket_name, key):
...
def get_us_states(file_path: str) -> gpd.GeoDataFrame:
    """
...
# # Define the eta calculation function
# def calculate_eta(swe_change, threshold=T, k_value=k):
...
def encode(dataset, variable, output_path, time_step, scale, geojson_path, downsample_factor=1, rotate=False):
...
# # Get first dataset
# dataset1 = get_data(s3_filepath="snow-observing-systems/Missouri Open Loop sample output/LIS_HIST_201903090000.d01.nc")

# # Get second dataset
# dataset2= get_data(s3_filepath="snow-observing-systems/Missouri Open Loop sample output/LIS_HIST_201903100000.d01.nc")

# Load the Missouri River Basin boundary
mo_basin = get_missouri_basin(file_path="WBD_10_HU2_Shape/Shape/WBDHU2.shp")

# Load the US states boundary
conus = get_us_states(file_path="https://www2.census.gov/geo/tiger/GENZ2018/shp/cb_2018_us_state_20m.zip")

##################
#Combined dataset#
##################
# # Generate combined dataset
# combined_output_file = generate_combined_dataset(dataset1, dataset2)

# # Upload data
# s3.upload_file(
#     combined_output_file, credentials['s3_bucket'], combined_output_file, Config=config
# )
combined_output_file = 'LIS_dataset_20190310.nc'

# Load the combined dataset
combined_dataset = get_data(s3_filepath=f"snow-observing-systems/{combined_output_file}")
...
swe_layer_encoded, top_left, top_right, bottom_left, bottom_right = encode(
...
# swe_output_file = generate_swe_difference(ds=combined_dataset)

# # Upload data
# s3.upload_file(
#     swe_output_file, credentials['s3_bucket'], swe_output_file, Config=config
# )

swe_output_file = 'Efficiency_SWE_Change_20190310.nc'

# Load the clipped dataset
eta5_file = get_data(s3_filepath=f"snow-observing-systems/{swe_output_file}")

# Select the eta5 variable for a specific time step (e.g., first time step)
eta5_data = eta5_file["eta5"].isel(time=1)

eta5_layer_encoded, _, _, _, _ = encode(
    dataset=eta5_file,
    variable='eta5',
...
# surfacetemp_output_file = generate_surface_temp(ds=combined_dataset)

# # Upload data
# s3.upload_file(
#     surfacetemp_output_file, credentials['s3_bucket'], surfacetemp_output_file, Config=config
# )

surfacetemp_output_file = 'Efficiency_SurfTemp_20190310.nc'

# Load the clipped dataset
eta0_file = get_data(s3_filepath=f"snow-observing-systems/{surfacetemp_output_file}")

# Select the eta0 variable for a specific time step (e.g., first time step)
eta0_data = eta0_file["eta0"].isel(time=1)

eta0_layer_encoded, _, _, _, _ = encode(
    dataset=eta0_file,
    variable='eta0',
...
# sensor_gcom_output_file = generate_sensor_gcom(ds=combined_dataset)

# # Upload data
# s3.upload_file(
#     sensor_gcom_output_file, credentials['s3_bucket'], sensor_gcom_output_file, Config=config
# )

sensor_gcom_output_file = 'Efficiency_Sensor_GCOM_20190310.nc'

# Load the clipped dataset
eta2_file_GCOM = get_data(s3_filepath=f"snow-observing-systems/{sensor_gcom_output_file}")

# Select the eta2 variable for a specific time step (e.g., first time step)
eta2_data_GCOM = eta2_file_GCOM["eta2"].isel(time=1)

eta2_gcom_layer_encoded, _, _, _, _ = encode(
...
# sensor_capella_output_file = generate_sensor_capella(ds=combined_dataset)

# # Upload data
# s3.upload_file(
#     sensor_capella_output_file, credentials['s3_bucket'], sensor_capella_output_file, Config=config
# )

sensor_capella_output_file = 'Efficiency_Sensor_Capella_20190310.nc'

# Load the clipped dataset
eta2_file_Capella = get_data(s3_filepath=f"snow-observing-systems/{sensor_capella_output_file}")

# Select the eta2 variable for a specific time step (e.g., first time step)
eta2_data_Capella = eta2_file_Capella["eta2"].isel(time=1)

eta2_capella_layer_encoded, _, _, _, _ = encode(
...
# gcom_combine_multiply_output_file = combine_and_multiply_datasets(
#     ds = combined_dataset,
#     eta5_file=eta5_file,
#     eta0_file=eta0_file,
...
# # Upload data
# s3.upload_file(
#     gcom_combine_multiply_output_file, credentials['s3_bucket'], gcom_combine_multiply_output_file, Config=config
# )
gcom_combine_multiply_output_file = 'Combined_Efficiency_Weighted_Product_GCOM_20190310.nc'

# Load the clipped dataset
gcom_dataset= get_data(s3_filepath=f"snow-observing-systems/{gcom_combine_multiply_output_file}")
...
gcom_eta_layer_encoded, _, _, _, _ = encode(
...
# capella_combine_multiply_output_file = combine_and_multiply_datasets(
#     ds = combined_dataset,
#     eta5_file=eta5_file,
#     eta0_file=eta0_file,
...
# s3.upload_file(
#     capella_combine_multiply_output_file, credentials['s3_bucket'], capella_combine_multiply_output_file, Config=config
# )
capella_combine_multiply_output_file = 'Combined_Efficiency_Weighted_Product_Capella_20190310.nc'

# Load the clipped dataset
capella_dataset= get_data(s3_filepath=f"snow-observing-systems/{capella_combine_multiply_output_file}")

# Select the combined_eta variable for a specific time step (e.g., first time step)
capella_eta = capella_dataset["combined_eta"].isel(time=1)

capella_eta_layer_encoded, _, _, _, _ = encode(
...
#Final ETA#
###########
# final_eta_output_file, gcom_tracks, capella_tracks = process(ds=combined_dataset, mo_basin=mo_basin, gcom_ds=gcom_dataset, capella_ds=capella_dataset)

# # Upload data
# s3.upload_file(
#     final_eta_output_file, credentials['s3_bucket'], final_eta_output_file, Config=config
# )

final_eta_output_file = 'Reward_20190310.geojson'

# Load the clipped dataset
# final_eta_gdf = get_data_geojson(s3_filepath=f"snow-observing-systems/{final_eta_output_file}")
...
#######################
# output_geojson = find_optimal_solution(final_eta_gdf=final_eta_gdf)

# # Upload data
# s3.upload_file(
#     output_geojson, credentials['s3_bucket'], output_geojson, Config=config
# )
...
selected_cells_gdf = get_data_geojson(s3_filepath=f"snow-observing-systems/{output_geojson}") #gpd.read_file("Selected_Cells_Optimization.geojson")  # Selected cells
# mo_basin = gpd.read_file("WBD_10_HU2_Shape/Shape/WBDHU2.shp")  # Missouri Basin boundary
