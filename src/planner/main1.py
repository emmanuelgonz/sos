...
class Environment(Observer):
    ...
    *The Environment object class inherits properties from the Observer object class in the NOS-T tools library*
    ...
    #     lat_coords = np.linspace(37.024602, 49.739086, 29)  # 76)
    #     lon_coords = np.linspace(-113.938141, -90.114221, 40)  # 142)
    ...
    #     ).replace("-", "")
    #     # print(last_date_ds1) #20190309
    #     # print(last_date_ds2) #20190310
    ...
    #     # print(
    #     #     "Combined dataset dimensions:", combined_dataset.dims
    #     # )  # Frozen({'time': 2, 'y': 29, 'x': 40})
    ...
    # def process(self, gcom_ds, snowglobe_ds):
    #     # , ds, eta5_file, eta0_file, eta2_file, weights, output_file):
    #     """
    #     Combine three datasets by applying weights and performing grid-cell multiplication.
    ...
    #         None
    #     """
    #     logger.info("Generating final effiency dataset.")
    ...
    #     # gcom_ds = get_data(
    #     #     s3_filepath="snow-observing-systems/Combined_Efficiency_Weighted_Product_GCOM_20190310.nc"
    #     # )
    #     # snowglobe_ds = get_data(
    #     #     s3_filepath="snow-observing-systems/Combined_Efficiency_Weighted_Product_Capella_20190310.nc"
    #     # )
    ...
    #     gcom_union = gcom_tracks.unary_union
    #     if gcom_union.geom_type == "MultiPolygon":
    #         gcom_geometries = list(gcom_union.geoms)  # Extract individual polygons
    #     else:
    #         gcom_geometries = [gcom_union]  # Wrap single Polygon in a list

    #     snowglobe_union = ground_tracks.unary_union
    #     if snowglobe_union.geom_type == "MultiPolygon":
    #         snowglobe_geometries = list(snowglobe_union.geoms)
    #     else:
    #         snowglobe_geometries = [snowglobe_union]

    #     snowglobe_mask = snowglobe_eta.rio.clip(
    #         snowglobe_geometries, drop=False
    #     ).notnull()
    #     gcom_mask = gcom_eta.rio.clip(gcom_geometries, drop=False).notnull()

    #     # print(snowglobe_mask)
    #     # print(gcom_mask)

    #     # Compute final_eta values
    #     final_eta = xr.full_like(snowglobe_eta, np.nan)  # Initialize with NaN
    #     final_eta = final_eta.where(snowglobe_mask)  # Retain only Capella-covered areas
    #     final_eta = final_eta.where(
    #         ~gcom_mask, snowglobe_eta - gcom_eta
    #     )  # Subtract where overlap
    #     final_eta = final_eta.where(
    #         ~snowglobe_mask, snowglobe_eta
    #     )  # Retain Capella-only values
    #     final_eta = final_eta.where(snowglobe_mask, np.nan)

    #     # Extract last available time from the dataset
    #     final_last_date = snowglobe_eta["time"].values

    #     # Assign time coordinate to the final dataset
    #     final_eta = final_eta.assign_coords(time=final_last_date)

    #     # Convert final_eta to DataFrame before converting to GeoDataFrame
    ...
    #     # Add metadata with the last time step
    #     final_eta_gdf["time"] = pd.Timestamp(final_last_date)

    #     # Ensure everything has the same CRS
    #     final_eta_gdf = final_eta_gdf.to_crs(epsg=4326)
    #     mo_basin = mo_basin.to_crs(epsg=4326)
    ...
    #     # final_eta_gdf = final_eta_gdf.sjoin(mo_basin, how="inner", predicate="intersects")

    #     # Save the dataset after proper clipping
    #     output_file = (
    ...
    #     # print(
    #     #     f"Final eta GeoDataFrame saved to '{output_file}' with time: {final_last_date}"
    #     # )
    #     logger.info("Generating final effiency dataset successfully completed.")
    ...
    #     # Compute orbit tracks without parallel processing
    #     orbit_tracks = pd.concat(
    #         [
    #             collect_orbit_track(
    #                 satellite=satellite,
    ...
    #     # Compute ground tracks without parallel processing
    #     ground_tracks = pd.concat(
    #         [
    #             compute_ground_track(
    #                 satellite=satellite,
    ...
    def process(self, gcom_ds, snowglobe_ds, mo_basin):
        """
        Combine three datasets by applying weights and performing grid-cell multiplication.
        ...
        # Create masks for ground tracks
        logger.info("Creating masks for ground tracks.")
        gcom_union = gcom_tracks.unary_union
        if gcom_union.geom_type == "MultiPolygon":
            gcom_geometries = list(gcom_union.geoms)  # Extract individual polygons
        else:
        ...
        final_eta_df = final_eta.to_dataframe(name="final_eta").reset_index()
        logger.info(
            "Computing final efficiency values successfully completed."
        )  # Good up to here
        ...
        final_eta_gdf = final_eta_gdf.to_crs(epsg=4326)
        # mo_basin = mo_basin.to_crs(epsg=4326)
        mo_basin = gpd.GeoDataFrame(mo_basin, columns=["geometry"]).set_crs(epsg=4326)
        logger.info(f"final_eta_gdf: {type(final_eta_gdf)}")
        logger.info(f"mo_basin: {type(mo_basin)}")
    ...
    def on_change(self, source, property_name, old_value, new_value):
        if property_name == "time":
            ...
            if change:
                ...
                selected_cells_gdf = gpd.read_file(geojson_path)
